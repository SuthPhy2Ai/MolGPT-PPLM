...
from uncertainty import UncertaintyMetrics, StepUncertainty, SequenceUncertainty
...
def sample_with_uncertainty(model, x, steps, temperature=1.0, sample=False, top_k=None,
    ...
    for k in range(steps):
        ...
        # Apply temperature
        if temperature != 1.0:
            # For Dirichlet, we scale alpha by temperature
            alpha_temp = 1.0 + (alpha_last - 1.0) / temperature
            S_temp = torch.sum(alpha_temp, dim=-1, keepdim=True)
            probs = alpha_temp / S_temp
